...
Naive, whenever object used, immediately using if checks, 
Results in scattered if checks and a lot of places for changes if object properties changed.

Proxies simplify by centrally handling these in one place, define rules in one place, and each time the object is used, rules are already applied.
